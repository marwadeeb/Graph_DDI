...
### GNN (planned)
GNNExplainer is planned for the GNN model once Laure's trained weights are available.
It produces per-edge and per-node feature importance maps, attributing predictions to
specific graph neighbours and node properties.
...
### Mitigation
- Report per-category AUC in GNN error analysis (TM6) once the model is available.
- Consider degree-stratified evaluation (low / medium / high degree buckets).
